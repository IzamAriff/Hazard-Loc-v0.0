"""
HazardCNN (ResNet-18) Specific Loss Function: Focal Loss
Independent file - does not interfere with src/utils/losses.py
Per dissertation: Baseline CNN model benefits from class imbalance handling
"""
...
class HazardCNNFocalLoss(nn.Module):
    """
    Focal Loss for ResNet-18 baseline (native 2D classification)
    Addresses class imbalance in hazard detection
    """
    def __init__(self, alpha=None, gamma=2.0, reduction='mean'):
        ...
        print(f"✓ HazardCNNFocalLoss initialized (α={'class-weighted' if alpha is not None else 'None'}, γ={gamma})")
    ...
    def forward(self, inputs, targets):
        ...
        """
        if inputs.dim() != 2:
            raise ValueError(f"HazardCNNFocalLoss expects 2D input [B, C], got {inputs.shape}")
        ...
        if self.alpha is not None:
            ...
            # The -log_pt is the cross-entropy loss
